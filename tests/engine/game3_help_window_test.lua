-- Sections 5.5/5.6 of docs/game3/e10-opcode-spec.md: the help MESSAGE window
-- behind `loadhelp` / `unloadhelp`. Unwired (the dispatch cases are the
-- Finisher's), so this suite pins the contract they will call: open/close,
-- pret's idempotency, the window geometry, and that resolution of the opcode's
-- word argument is deliberately NOT this module's job.
--   luajit tests/engine/game3_help_window_test.lua
...
-- The engine canvas is the GBA grid, so pret's rect maps 1:1.
...
-- pret PrintHelpMessageText text placement (src/help_message.c:95-98).
...
-- pret PrintTextOnHelpMessageWindow takes an already-resolved pointer; the
-- engine handler resolves the opcode's word with resolve_text (ops_a.lua:36-45)
-- and passes a string. This module must not interpret pointers itself.
...
-- Re-showing replaces the content (pret's second DrawHelpMessageWindowWithText).
...
-- Non-string input degrades to empty content rather than erroring (the handler
-- may pass nil when resolve_text misses; pret falls back to ctx->data[0]).
