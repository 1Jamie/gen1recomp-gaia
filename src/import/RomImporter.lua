...
-- CacheContract's Gold required-file list re-imports exactly the caches that
-- lack the stage, the same per-version mechanism used for Yellow #439/#557.
...
local MARKER_PATH = CacheContract.MARKER_PATH
...
local function purgeSaveDirCache()
  ...
  for _, version in ipairs(GameVersion.ORDER) do
    ...
    local probe = CacheContract.requiredFiles(version)[1]
    if saveDirHas(prefix .. MARKER_PATH) or saveDirHas(prefix .. probe) then
...
function RomImporter.isReady(version)
  ...
  local backing = {
    ...
    getInfo = function(path)
      return CacheFs.existsAt(path) and { type = "file" } or nil
    end,
  }
  if love and love.filesystem then
    backing.getRealDirectory = love.filesystem.getRealDirectory
    backing.getSource = love.filesystem.getSource
  end
  return CacheContract.inspect(version, backing, { allowSource = true }) ~= nil
end
