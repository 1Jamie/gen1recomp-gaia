...
function Hit.applySetDamage(M, dmg, flags)
  ...
  if hung == "endured" then
    M:say(ad:displayName(target) .. " ENDURED\nthe hit!")
...
local function pre_checks(M)
  ...
  if eff == E.SPIT_UP and M:isProtected() then
    ...
    if (user.expStockpile or 0) <= 0 then
      M:say("But it failed to SPIT UP\na thing!")
    else
      user.expStockpile = 0
      M:say(M.tname .. "\nprotected itself!")
    end
...
local function flags_immune(M, info)
  ...
  if ad:abilityOf(target) == "LEVITATE" and mt == Types.ID.GROUND then
    M:say(M.tname .. " makes GROUND\nmoves miss with LEVITATE!")
    return true
  end
...
local function wonder_guard(M, info)
  ...
  M:say(M.tname .. " avoided\ndamage with WONDER GUARD!")
  return true
end
...
local function hit_once(M, opts)
  ...
  if info.critical then M:say("A critical hit!") end
...
function Hit.run(M)
  ...
  if eff == E.EXPLOSION then
    ...
    if hung == "endured" then M:say(M.tname .. " ENDURED\nthe hit!")
  ...
  for i = 1, nHits do
    ...
    local _, info, status = hit_once(M, {
      ...
      onAfterAnim = (i == 1 and M.brokeWall) and function() M:say("The wall shattered!") end or nil,
    })
  ...
  if isMulti and landed > 0 then
    ...
    M:say(string.format("Hit %d time(s)!", landed))
  end
  if landed == 0 then
    M.noEffect = true
    if eff == E.ROLLOUT then
...
function Hit.beatUp(M)
  ...
  for i, mon in ipairs(party) do
    ...
    if mon and hp > 0 and (mon.species or 0) ~= 0 and not mon.isEgg and (status == nil or status == 0) then
      ...
      if crit then M:say("A critical hit!") end
      if hung == "endured" then M:say(M.tname .. " ENDURED\nthe hit!")
